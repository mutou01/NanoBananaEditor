...
本项目对生成图片的**尺寸**和**文件大小**进行了严格控制，确保符合电商平台的上传要求（通常要求 < 2MB）。

### 尺寸控制策略

图生图模式提供三种尺寸选项，均限制最大边不超过 800px：

| 选项 | 实际尺寸 | 提示词增强 | 适用场景 |
|------|----------|------------|----------|
| **1:1 (默认)** | 800×800 | `output size 800x800 pixels max` | 正方形主图 |
| **3:4** | 600×800 | `output size 600x800 pixels max` | 竖向详情图 |
| **Auto** | 保持比例，最大边 800px | `maximum dimension 800 pixels` | 保持原图比例 |

### 文件大小控制 (< 2MB)

即使 API 返回了超大图片，系统会在客户端自动压缩：

**压缩流程**：
API 返回图片 ↓ 检查文件大小 ↓

2MB? → 按比例缩小尺寸 (最大边 800px → 如仍大则继续缩小) ↓ Canvas 重绘 (高质量缩放) ↓ 检查大小 ↓ 2MB? → 继续降低质量/尺寸 ↓ 保存并显示

**核心代码** (`src/utils/imageUtils.ts`):
```typescript
export async function compressImageToSize(
  base64OrUrl: string,
  maxSizeMB: number = 2,      // 最大 2MB
  mimeType: string = 'image/png',
  maxDimension: number = 800  // 最大边 800px
): Promise<string> {
  // 1. 检查当前大小
  // 2. 如超过 2MB，计算缩放比例
  // 3. Canvas 重绘 (使用 high quality smoothing)
  // 4. 如仍超过限制，递归降低质量/尺寸
  // 5. 返回压缩后的图片
}
```

## 适配器集成: 所有适配器在返回图片前都会调用压缩函数：
```typescript
// OpenAI Adapter
...
## 控制台输出示例
```typescript
[Image Compression] Current size: 3.12MB, target: 2MB
[Image Compression] Image too large (1254x1254), scaling to max 800px
[Image Compression] Final size: 1.45MB, dimensions: 800x800, scale: 0.64
```

## 技术细节
**压缩算法**：

✅ 优先降低尺寸（保持比例，最大边 800px）
✅ 高质量 Canvas 重绘（imageSmoothingQuality: 'high'）
✅ PNG 格式保持无损（仅降低尺寸）
✅ JPEG 格式可额外降低质量（最低 0.5）
✅ 最小缩放限制（50%），避免过度压缩
**保留质量**：
...
**注意事项**
默认 800×800：图生图默认使用 1:1 模式，输出 800×800 正方形图片
自动压缩：用户无需操作，系统自动确保图片 < 2MB
原始比例：Auto 模式保持源图比例，仅限制最大边
压缩不可逆：压缩后的图片无法恢复原始分辨率
性能影响：压缩通常在 100-300ms 内完成，对体验影响极小